...
const TaskList = ({ tasks, onAddTask }) => {
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-4 h-96">
      ...
      {/* Task List */}
      {tasks.length === 0 ? (
        <div className="text-center text-gray-500">No tasks available</div> // Show message if no tasks
      ) : (
        <ul className="space-y-2">
          {tasks.map((task, index) => (
            <li
              key={index}
              className={`flex items-center justify-between px-3 py-2 rounded-lg ${
                task.completed ? "bg-green-50" : "hover:bg-gray-100 transition-colors"
              }`}
            >
              {/* Task Number and Name */}
              <div className="flex items-center space-x-3">
              ...
              <button
                className={`p-1 rounded-full ${
                  task.completed ? "text-green-400" : "text-gray-500 hover:bg-gray-100"
                }`}
              >
                {task.completed ? (
                  <Icon icon="fluent-mdl2:completed-solid" height={22} width={22} />
                ) : (
                  <Icon icon="fluent-mdl2:completed" height={22} width={22} />
                )}
              </button>
