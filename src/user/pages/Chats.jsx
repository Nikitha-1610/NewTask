...
import GroupChat from "../../common/components/GroupChat";
...
const ChatApp = () => {
  ...
  const [filteredGroups, setFilteredGroups] = useState([{ name: "web team", id: "654123" }]);
  const [filteredPinnedContacts, setFilteredPinnedContacts] = useState([]);
  const [selectedContact, setSelectedContact] = useState(null);  // Contact or Group
  ...
  const getUsers = async () => {
    try {
      ...
      const filteredData = response.data.message.filter(
  ...
  // Handle selecting a contact (user)
  const handleContactClick = (contact) => {
    setSelectedContact({ type: "user", contact });
  };

  // Handle selecting a group
  const handleGroupClick = (group) => {
    setSelectedContact({ type: "group", group });
  };
