...
const NewCompleted = () => {
  ...
  useEffect(() => {
    ...
    axiosInstance
      ...
      .then((response) => {
        console.log("responseCompleted", response);
        if (response.data && response.data.message) {
          const { completedTasks } = response.data.message;
          setTaskAssign({
            completedTasks,
          });
        } else {
  ...
  const filteredColumns = [
    {
      ...
      color: "green",
      tasks: filterTasks(taskAssign.completedTasks),
      path: "completed",
      status: "Completed",
