      ...
      try {
        ...
        if (Array.isArray(data.message)) {
          ...
          const taggedEmployees =
            JSON.parse(localStorage.getItem("taggedEmployees")) || [];

          // Filter out tagged employees
          let untaggedUsers = data.message.filter(
            (user) => !taggedEmployees.includes(user.name)
          );
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          setFilteredUsers((prevFilteredUsers) =>
            prevFilteredUsers.filter(
              (user) => user.name !== selectedEmployee.name
            )
          );
        ...
        toast.error(
          error.response?.data?.message ||
            "Failed to tag the employee. Please try again."
        );
      }
    } else {
  ...
  return (
    <div className="py-0 sm:px-5 px-0">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-start items-center gap-4 mb-6 sm:w-auto w-32">
        ...
        >
          <option className="w-[100px]" value="all">
            All Positions
          </option>
          ...
          {filteredUsers.length > 0 ? (
            filteredUsers.map((user, index) => (
              <div
                ...
                style={{
                  borderColor:
                    selectedEmployee?.name === user.name ? "blue" : "",
