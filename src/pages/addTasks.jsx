...
const AddTasks = () => {
  ...
  const handleClose = () => {
    // Close the success message and redirect to the same page
    setIsSubmitted(false);
    navigate("/addtasks"); // Redirect back to the form page
  };
  ...
  return (
    <div className="add-tasks-container flex flex-col md:flex-row h-full p-4 relative">
      {/* Left Container */}
      <div className="left-container flex flex-col gap-6 p-4 border-0 rounded-md w-full md:w-[713px]">
        <TaskForm taskData={taskData} setTaskData={setTaskData} />
      </div>

      {/* Right Container for Buttons */}
      <div className="absolute top-30 right-6 flex gap-4">
        <Link  onClick={handleAddTaskClick}
          to="/addtasks"
          className="add-task-btn flex items-center gap-2 bg-[#01C2B5] text-white p-3 rounded-[19px] w-[150px] h-[45px] shadow-lg hover:bg-[#01B2A5] font-[600] font-sans"
        >
        ...
        <Link
          to="/filter"
          className="filter-btn flex items-center gap-2 text-[#475569] p-3 rounded-md shadow-lg hover:bg-transparent font-sans font-[600]"
        >
      ...
      {/* Success Message */}
      {isSubmitted && (
  <div className="success-message fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-50">
    <div className="success-container flex flex-col items-center gap-10 p-10 w-[500px] font-sans bg-white rounded-lg shadow-lg relative sm:w-[400px] sm:p-8 border-2 border-black">
      {/* Close Icon */}
  ...
  >
    Great
      ...
      <button   onClick={handleSubmit}
        className="submit-btn absolute bottom-0 right-4 rounded-[var(--Spacing-8,8px)] bg-[#01C2B5] w-[121px] py-[10px] px-[12px] text-white shadow-lg flex items-center justify-center hover:bg-[#019F97] font-sans font-[600]"
      >
          Submit
        </button>
      </div>
