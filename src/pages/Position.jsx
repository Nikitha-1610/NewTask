...
const Position = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (selectedEmployee && selectedTeamLead) {
      try {
        const response = await fetch(
          "https://3qhglx2bhd.execute-api.us-east-1.amazonaws.com/employee/tag",
          {
            method: "PUT",
            headers: {
          ...
          {filteredUsers.length > 0 ? (
            filteredUsers.map((user, index) => (
              <div
                ...
                style={{
                  backgroundColor:
                    selectedEmployee?.name === user.name ? "#98fb98" : "white",
      ...
      {show && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 shadow-lg rounded z-50 w-[90%] max-w-lg sm:max-w-md md:max-w-md lg:max-w-md h-80 text-center border-2 border-gray-500">
            ...
            >
              <option value="" className="truncate w-24 text-base sm:text-lg">
                Select Team Lead
              </option>
